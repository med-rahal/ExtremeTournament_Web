...
use App\Form\AddCommentBackType;
...
class PublicationController extends AbstractController
{
    ...
    public function listepubF(Request $request, PaginatorInterface $paginator)
    {
        ...
        return $this->render('blog/listepubF.html.twig', array('publications' => $pagination,
            'commentaires'=>$commentaires));
    }
    ...
    public function reportcomment($id,\Swift_Mailer $mailer)
    {
       ...
       if( $commentaires->getNbrReports()>3){
        ...
        $message = (new \Swift_Message('the comment with this id is reported'))
            ...
            ->setTo('xtreametournamnet@gmail.com');
            $img = $message->embed(\Swift_Image::fromPath('images/logo.png'));
            $message->setBody(
                $this->renderView(
                    'publication/report.html.twig',
                    ['name' => $name ,'text'=>$text,'date'=>$date,'img'=> $img]
                ),
                'text/html'
            );
        $mailer->send($message);
    ...
    public function tristparcommennts()
    {
        ...
        return $this->render('home\listepubtri.html.twig', array(
 ...
 */
    ...
    public function addComment(Request $request)
    {
        ...
        $form = $this->createForm(AddCommentBackType::class, $commentaires);
        $form->add('Ajouter',SubmitType::class);
        $form->handleRequest($request);
        $commentaires->setIdUser($user);
        $commentaires->setDateComment(new \DateTime());
        $commentaires->setNbrReports(0);
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
